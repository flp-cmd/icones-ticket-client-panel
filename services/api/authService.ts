import { AuthTokens, AuthUser, LoginCredentials, LoginResponse, TwoFactorResponse } from '@/types/auth';
...
class AuthService {
  ...
  async verifyTwoFactor(twoFactorToken: string, twoFactorCode: string): Promise<TwoFactorResponse> {
  ...
  async verifyTwoFactorTime(twoFactorCode: string): Promise<{ success: boolean }> {
  ...
  async confirmTwoFactorSetup(request: TwoFactorSetupConfirmRequest): Promise<TwoFactorSetupConfirmResponse> {
    return httpService.post<TwoFactorSetupConfirmResponse>('/auth/2fa/setup/confirm', request);
  }
